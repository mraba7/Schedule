...
internal fun ClassPage(config:Config,section:String,commit:(Config)->Unit) {
    ...
    LazyColumn(modifier=Modifier.fillMaxSize(),contentPadding=PaddingValues(20.dp),verticalArrangement=Arrangement.spacedBy(12.dp)) {
        ...
        item{OutlinedButton(onClick={openStudio(c,"shortcuts",section)},modifier=Modifier.fillMaxWidth()){Text("اختصارات الفصل · ملفات وروابط ومجلدات")}}
        item{OutlinedButton(onClick={openStudio(c,"classroom",section)},modifier=Modifier.fillMaxWidth()){Text("إدارة الحصة · الطلاب والتجارب والفهم")}}
